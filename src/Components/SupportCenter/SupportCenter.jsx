...
const SupportCenter = () => {
  ...
  const supportData = useMemo(() => [
    { id: 1, user: { name: 'Theresa Webb', email: 'bill.sanders@example.com', avatar: 'https://i.pravatar.cc/150?u=theresa' }, topic: 'I have issue with my dashbaord', status: 'Solved' },
    { id: 2, user: { name: 'Marvin McKinney', email: 'tim.jennings@example.com', avatar: 'https://i.pravatar.cc/150?u=marvin' }, topic: 'I dont know how to say', status: 'Dismissed' },
    { id: 3, user: { name: 'Courtney Henry', email: 'georgia.young@example.com', avatar: 'https://i.pravatar.cc/150?u=courtney' }, topic: 'What is the use of your', status: 'Pending' },
    { id: 4, user: { name: 'Steve Herd', email: 'sarah.c@vesioh.com', avatar: 'https://i.pravatar.cc/150?u=steve' }, topic: 'Help me with the webstie', status: 'Pending' },
    { id: 5, user: { name: 'Jane Cooper', email: 'jane.cooper@example.com', avatar: 'https://i.pravatar.cc/150?u=jane' }, topic: 'Billing issue', status: 'Solved' },
    { id: 6, user: { name: 'Cody Fisher', email: 'cody.fisher@example.com', avatar: 'https://i.pravatar.cc/150?u=cody' }, topic: 'Login problem', status: 'Pending' },
  ], []);

  const filteredData = useMemo(() => {
    return supportData.filter(ticket => {
      ...
      const matchesStatus = activeStatusFilter === 'Status' || ticket.status === activeStatusFilter;
      return matchesSearch && matchesStatus;
    });
  }, [searchTerm, activeStatusFilter, supportData]);
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-[#1E1E2D] rounded-[32px] shadow-2xl w-full max-w-[280px] overflow-hidden animate-in zoom-in duration-200">
            ...
            <div className="flex flex-col">
              ...
              <button className="flex items-center gap-3 px-6 py-4 text-sm font-bold text-[#10B981] hover:bg-[#10B981]/5 transition-all border-b border-gray-50 dark:border-gray-800">
                <CheckCircle size={18} /> Mark as Solved
              </button>
              <button className="flex items-center gap-3 px-6 py-4 text-sm font-bold text-gray-400 hover:bg-gray-50 dark:hover:bg-[#2D2D3F] transition-all">
